<x-app-layout :project="$project">
    ...
    <x-slot name="header">
        <div class="flex items-center gap-2.5">
            <a href="{{ route('epics.index', $project) }}" class="text-gray-400 hover:text-gray-700 transition">
            ...
            <h2 class="text-[15px] font-semibold text-gray-900">{{ $epic->name }}</h2>
        </div>
        <div class="flex items-center gap-1.5">
            <button onclick="document.getElementById('edit-epic-dialog').showModal()" class="text-gray-400 hover:text-gray-700 transition p-1.5 rounded hover:bg-gray-100" title="Edit epic">
                ...
                @else
                    <div class="card overflow-hidden divide-y divide-gray-100">
                        @foreach($epic->tickets as $ticket)
                            <a href="{{ route('tickets.show', [$project, $ticket]) }}" class="group flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition">
                                @php
                                    $priorityColors = [
                                        'urgent' => 'bg-red-500',
                                        'high' => 'bg-orange-500',
                                ...
                                <span class="text-[13px] text-gray-800 group-hover:text-gray-900 transition flex-1 truncate">{{ $ticket->title }}</span>
                                <span class="text-[11px] text-gray-500 px-1.5 py-0.5 rounded bg-gray-100 flex-shrink-0">{{ $ticket->column->name }}</span>
                                @if($ticket->assignee)
        ...
        <div class="fixed inset-0 flex items-start justify-center pt-[8vh] p-4 pointer-events-none">
            <div class="bg-white border border-gray-200 rounded-xl shadow-2xl shadow-gray-900/10 w-full max-w-3xl pointer-events-auto flex flex-col max-h-[80vh]">
                <form method="POST" action="{{ route('epics.tickets.store', [$project, $epic]) }}" class="flex flex-col flex-1 min-h-0">
                    ...
                    <div class="flex items-center justify-end gap-2 px-4 py-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                        <button type="button" onclick="document.getElementById('create-ticket-dialog').close()" class="px-3 py-1.5 text-[12px] font-medium text-gray-500 hover:text-gray-800 transition">Cancel</button>
    ...
    <dialog id="edit-epic-dialog" class="backdrop:bg-black/80 backdrop:backdrop-blur-sm bg-transparent p-0 m-0 fixed inset-0 w-full h-full max-w-none max-h-none">
        <div class="fixed inset-0 flex items-start justify-center pt-[10vh] p-4 pointer-events-none">
            <div class="bg-white border border-gray-200 rounded-xl shadow-2xl shadow-gray-900/10 w-full max-w-lg pointer-events-auto">
                <form method="POST" action="{{ route('epics.update', [$project, $epic]) }}" class="flex flex-col">
                    ...
                    <div class="px-5 pt-5 pb-4 space-y-4">
                        ...
                        <textarea name="description" rows="3" placeholder="Describe the goal of this epic..."
                                  class="w-full bg-transparent border-0 text-[14px] text-gray-700 placeholder-gray-300 focus:ring-0 p-0 resize-none focus:outline-none leading-relaxed">{{ $epic->description }}</textarea>
                            ...
                            <div class="flex items-center gap-1.5">
                                @foreach(['#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#6366f1'] as $color)
                                    <label class="cursor-pointer">
                                        ...
                                        <div class="w-5 h-5 rounded-full border-2 border-transparent peer-checked:border-gray-900/50 transition" style="background: {{ $color }}"></div>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                        <button type="button" onclick="if(confirm('Delete this epic? Issues will be kept.')) document.getElementById('delete-epic-form').submit()" class="text-[12px] text-red-500 hover:text-red-600 font-medium transition">Delete</button>
                        <div class="flex items-center gap-2">
                            <button type="button" onclick="document.getElementById('edit-epic-dialog').close()" class="px-3 py-1.5 text-[12px] font-medium text-gray-500 hover:text-gray-800 transition">Cancel</button>
