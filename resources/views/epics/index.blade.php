<x-app-layout :project="$project">
    ...
    <x-slot name="header">
        <div class="flex items-center gap-2.5">
            <h2 class="text-[15px] font-semibold text-gray-900">Epics</h2>
    ...
    <div class="p-6 py-10 max-w-3xl mx-auto">
        @if($project->epics->isEmpty())
            <div class="text-center py-24">
                <div class="w-14 h-14 rounded-2xl bg-gray-100 border border-gray-200 flex items-center justify-center mx-auto mb-5">
        ...
        @else
            <div class="space-y-2">
                @foreach($project->epics as $epic)
                    <a href="{{ route('epics.show', [$project, $epic]) }}" class="group block card hover:border-gray-300 transition-all duration-200">
                        <div class="flex items-center gap-4 px-5 py-4">
                            <div class="w-1 h-8 rounded-full flex-shrink-0" style="background: {{ $epic->color }}"></div>
                            <div class="flex-1 min-w-0">
                                <h3 class="text-[13px] font-semibold text-gray-900 group-hover:text-gray-700 transition truncate">{{ $epic->name }}</h3>
                                @if($epic->description)
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-2xl shadow-gray-900/10 w-full max-w-lg pointer-events-auto">
                <form method="POST" action="{{ route('epics.store', $project) }}" class="flex flex-col">
                    ...
                    <div class="flex items-center justify-end gap-2 px-4 py-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                        <button type="button" onclick="document.getElementById('create-epic-dialog').close()" class="px-3 py-1.5 text-[12px] font-medium text-gray-500 hover:text-gray-800 transition">Cancel</button>
