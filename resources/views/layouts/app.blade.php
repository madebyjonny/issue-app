...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-50 text-gray-800">
        <div class="h-screen flex overflow-hidden">
            {{-- Sidebar --}}
            <aside class="w-56 flex-shrink-0 bg-white border-r border-gray-200 dark:bg-[#111114] dark:border-white/[0.06] flex flex-col">
                {{-- Brand --}}
                ...
                <nav class="flex-1 px-3 py-3 overflow-y-auto space-y-5 text-[13px]">
                    ...
                    <div>
                        ...
                        <div class="space-y-0.5">
                            <a href="{{ route('projects.index') }}" class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition {{ request()->routeIs('projects.index') || request()->routeIs('projects.create') ? 'bg-gray-100 text-gray-900 dark:bg-white/[0.08] dark:text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/[0.06]' }}">
                    ...
                    <div>
                        ...
                        <div class="space-y-0.5">
                            @foreach($sidebarProjects as $sidebarProject)
                                ...
                                @endphp
                                <div class="group flex items-center gap-1 rounded-lg transition {{ $isCurrentProject ? 'bg-gray-100 dark:bg-white/[0.08]' : 'hover:bg-gray-100 dark:hover:bg-white/[0.06]' }}">
                                    <a href="{{ route('projects.board', $sidebarProject) }}" class="flex-1 flex items-center gap-2.5 px-2 py-1.5 {{ $isCurrentProject ? 'text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white' }}">
                                        <div class="w-4 h-4 rounded bg-gray-100 dark:bg-white/[0.08] flex items-center justify-center text-[9px] font-semibold text-gray-600 dark:text-gray-300 flex-shrink-0">
                                            {{ substr($sidebarProject->key, 0, 1) }}
                                        </div>
                                        {{ $sidebarProject->name }}
                                    </a>
                                    <a href="{{ route('projects.show', $sidebarProject) }}" class="p-1.5 mr-1 text-gray-400 opacity-0 group-hover:opacity-100 hover:text-gray-900 dark:hover:text-white transition" title="Settings">
                    ...
                    <div>
                        ...
                        <div class="space-y-0.5">
                            <a href="{{ route('projects.board', $project) }}" class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition {{ request()->routeIs('projects.board') ? 'bg-gray-100 text-gray-900 dark:bg-white/[0.08] dark:text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/[0.06]' }}">
                            ...
                            <a href="{{ route('epics.index', $project) }}" class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition {{ request()->routeIs('epics.*') ? 'bg-gray-100 text-gray-900 dark:bg-white/[0.08] dark:text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/[0.06]' }}">
                            ...
                            <a href="{{ route('projects.show', $project) }}" class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition {{ request()->routeIs('projects.show') ? 'bg-gray-100 text-gray-900 dark:bg-white/[0.08] dark:text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/[0.06]' }}">
                                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                Settings
                            </a>
                        </div>
                ...
                <div class="border-t border-gray-200 dark:border-white/[0.06] p-3 relative" x-data="{ open: false }">
                    <button @click="open = !open" @mouseenter="open = true" class="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/[0.06] focus:outline-none transition group">
                        ...
                        <div class="flex-1 min-w-0 text-left">
                            <p class="text-[13px] text-gray-700 dark:text-gray-300 truncate leading-tight">{{ auth()->user()->name }}</p>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 group-hover:text-gray-600 dark:text-gray-500 dark:group-hover:text-gray-300 transition" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9"/></svg>
                    </button>
                    ...
                    <div x-show="open"
                         ...
                         class="absolute bottom-full left-3 right-3 mb-1 bg-white border border-gray-200 dark:bg-[#1f1f23] dark:border-white/[0.08] rounded-lg shadow-xl dark:shadow-black/40 overflow-hidden"
                         style="display: none;">
                        <div class="px-3 py-2.5 border-b border-gray-100 dark:border-white/[0.06]">
                            <p class="text-[12px] text-gray-500 dark:text-gray-400 truncate">{{ auth()->user()->email }}</p>
                        </div>
                        <div class="py-1">
                            <a href="{{ route('profile.edit') }}" class="flex items-center gap-2.5 px-3 py-2 text-[13px] text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-white dark:hover:bg-white/[0.06] transition">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-white dark:hover:bg-white/[0.06] transition">
