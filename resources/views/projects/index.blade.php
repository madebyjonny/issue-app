        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                @foreach($projects as $project)
                    <a href="{{ route('projects.board', $project) }}" class="group block p-4 card hover:border-gray-300 transition-all duration-200">
                        <div class="flex items-start gap-3 mb-3">
                            <div class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center text-[11px] font-bold text-gray-500 ring-1 ring-gray-200 flex-shrink-0">
                                {{ $project->key }}
                            </div>
                            <div class="min-w-0">
                                <h3 class="text-[13px] font-semibold text-gray-900 group-hover:text-gray-700 transition truncate">{{ $project->name }}</h3>
